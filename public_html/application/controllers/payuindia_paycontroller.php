<?php
class PayuIndia_payController extends PaymentController{
	private $key_name="PayuIndia";
	private function getPaymentForm($order_id){
		...
		$frm=new Form('frmPayuIndia','frmPayuIndia');
		...
		$frm->addHiddenField('key', 'key', $payment_settings["merchant_id"]);
		...
		$frm->addHiddenField('surl', 'surl', Utilities::generateAbsoluteUrl('payuindia_pay','callback'));
		$frm->addHiddenField('Furl', 'Furl', Utilities::generateAbsoluteUrl('payuindia_pay','callback'));
...
/*echo($key.'|'.$txnid.'|'.$payment_gateway_charge.'|'.$order_payment_gateway_description.'|'.$firstname.'|'.$email.'|'.$udf1.'||||||||||'.$salt);
die();*/
