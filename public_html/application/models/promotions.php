...
class Promotions{
	...
	function addUpdatePromotion($data)
	{
		...
		elseif ($promotion_id > 0 && $record->update(array(
	...
	function getPromotion($promotion_id)
	{
		...
		$srch->joinTable('tbl_products', 'LEFT OUTER JOIN', 'tp.promotion_product_id = p.prod_id', 'p');
	...
	function getUserPromotion($promotion_id, $user_id, $type)
	{
		$promotion_id = intval($promotion_id);
   ...
   SUM(IF(`pclick_datetime`>CURRENT_DATE - INTERVAL 1 MONTH,`pclick_cost`,0)) monthly_cost",
		...
		$criterias['front']=isset($criterias['front'])?$criterias['front']:'';
		if ($criterias['front']) {
			$srch->addCondition('userBalance','>=',Settings::getSetting("CONF_MIN_WALLET_BALANCE"));
			$srch->addDirectCondition('(tp.promotion_start_date = "0000-00-00" OR tp.promotion_start_date <= DATE(NOW())) AND (tp.promotion_end_date = "0000-00-00" OR tp.promotion_end_date >= DATE(NOW())) AND CAST("'.date('H:i:s').'" AS time) BETWEEN `promotion_start_time` AND `promotion_end_time`'); 
			
			$srch->addCondition('tp.promotion_is_approved', '=', 1);
			$srch->addCondition('u.user_status', '=', 1);
			$srch->addCondition('u.user_is_deleted', '=', 0);
		...
		return $criterias["pagesize"] == 1 ? $this->db->fetch($rs) : $this->db->fetch_all($rs);
	}
	function updatePromotionStatus($promotion_id, $data_update = array())
	{
		...
		if ($this->db->update_from_array('tbl_promotions', $data_update, array(
			'smt' => '`promotion_id` = ?',
			'vals' => array(
				$promotion_id
			)
	...
	function getPromotionLogs($criterias,$pagesize=10)
	{
		$srch = new SearchBase('tbl_promotions_logs', 'tpl');
		$srch->joinTable('tbl_promotions', 'INNER JOIN', 'tpl.lprom_id = p.promotion_id', 'p');
		$srch->addCondition('promotion_is_deleted', '=', 0);
		foreach($criterias as $key => $val) {
			if (strval($val) == '') continue;
			switch ($key) {
	...
	function getPromotionForm()
	{
		...
		$symbol = empty(CONF_CURRENCY_SYMBOL_LEFT) ? CONF_CURRENCY_SYMBOL_RIGHT : CONF_CURRENCY_SYMBOL_LEFT;
		$frm = new Form('frmPromote');
		$frm->addHiddenField('', 'promotion_id', 0);
		$fld = $frm->addHtml('<div class="label label-info">' . Utilities::getLabel('M_Charges_from_promotion_wallet') . '</div><br/>', 'promotion_note');
		$fld->merge_caption = true;
		$frm->addRequiredField('<label><strong>' . Utilities::getLabel('M_Cost_Per_Click') . '</strong></label> (' . $symbol . ')', 'promotion_cost', null, 'promotion_cost', 'readonly');
		$frm->addRequiredField('<label>' . Utilities::getLabel('M_Product') . '</label>', 'prod_name', null, 'prod_name');
		$frm->addHiddenField('', 'promotion_product_id', 0, 'promotion_product_id');
		$frm->addTextBox('<label>' . Utilities::getLabel('M_Shop') . '</label>', 'shop_name', '', 'shop_name', ' readonly ');
		$frm->addHiddenField('', 'promotion_shop_id', 0, 'promotion_shop_id');
		$fldBudget = $frm->addRequiredField('<label>' . Utilities::getLabel('M_Budget') . '</label> (' . $symbol . ')', 'promotion_budget', null, 'promotion_budget');
		$fldBudget->requirements()->setFloatPositive(true);
		$fldBudget->requirements()->setCompareWith('promotion_cost', 'ge',Utilities::getLabel('M_Budget'));
		$fldBudget->html_after_field = ' ';
		$fldDuration = $frm->addSelectBox('<label>' . Utilities::getLabel('M_Duration') . '</label>', 'promotion_budget_period', $duration_freq_arr, 'W', '', '', 'promotion_budget_period');
		$fldBudget->attachField($fldDuration);
		$fld_sd = $frm->addRequiredField('<label>' . Utilities::getLabel('M_Start_Date') . '</label>', 'promotion_start_date', '', 'promotion_start_date', 'class="date-pick calendar" readonly ');
		$fld_ed = $frm->addRequiredField('<label>' . Utilities::getLabel('M_End_Date') . '</label>', 'promotion_end_date', '', 'promotion_end_date', 'class="date-pick calendar" readonly ');
		
		$fld_ed->requirements()->setCompareWith('promotion_start_date', 'ge',Utilities::getLabel('M_End_Date'));
		
		$fld = $frm->addRequiredField('<label>' . Utilities::getLabel('M_Preferred_Start_Time') . '</label>', 'promotion_start_time', '', '', 'class="time" readonly ');
		$fld = $frm->addRequiredField('<label>' . Utilities::getLabel('M_Preferred_End_Time') . '</label>', 'promotion_end_time', '', '', 'class="time" readonly ');
		...
		$frm->setExtra('class="siteForm web_form"');
		$frm->setTableProperties('width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl-twocell table_form_horizontal"');
	...
	function addPromotionAnalysisRecord($data = array() , $column = 'impressions')
	{
		...
		$onDuplicateKeyUpdate = array_merge($assign_fields, array(
			'lprom_' . $column => 'mysql_func_lprom_' . $column . '+1'
		));
		if ($column == "clicks") {
			if ($this->addPromotionClicksHistory($promotion_id)){
				$this->db->insert_from_array('tbl_promotions_logs', $assign_fields, true, array('IGNORE'), $onDuplicateKeyUpdate);
			}
		}else{
			$this->db->insert_from_array('tbl_promotions_logs', $assign_fields, true, array('IGNORE'), $onDuplicateKeyUpdate);
		}
	...
	private function addPromotionClicksHistory($promotion_id){
		...
		$record = new TableRecord('tbl_promotions_clicks');
		$record->assignValues($assign_fields);
	...
	{
		$srch = new SearchBase('tbl_promotions_clicks', 'tpc');
		$srch->joinTable('tbl_promotions', 'LEFT JOIN', 'tpc.pclick_promotion_id = tp.promotion_id', 'tp');
		$srch->doNotCalculateRecords();
		$srch->doNotLimitRecords();
		$srch->addGroupBy('tpc.pclick_promotion_id');
		$srch->addMultipleFields(array(
	...
	function addUpdatePromotionCharges($data)
	{
		...
		$record = new TableRecord('tbl_promotions_charges');
		$assign_fields = array();
		...
		if ($record->addNew()) {
			...
			$txnArray["utxn_comments"] = sprintf(Utilities::getLabel('M_Charges_for_promotion_from_duration') , $formatted_request_value, Utilities::formatDate($assign_fields['pcharge_start_date']) , Utilities::formatDate($assign_fields['pcharge_end_date']) , $assign_fields['pcharge_clicks']);
